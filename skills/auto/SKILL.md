...
- Add `.ba/auto.lock`, `.ba/auto.count`, `.ba/auto.waiting`, `.ba/logs/`, `.ba/cache/` and `.ba/security/` to `.gitignore` if missing.
...
- Roles are separated. You (orchestrator) never write product code or tests. **Writer** = agent `ba:implementer`. **Checkers** = agents `ba:verifier` and `ba:reviewer`. A checker never receives the writer's reasoning, only: issue text, KIND, file lists, branch.
...
- Never rewrite git history, never force-push, never rotate or print secrets: those are `needs-human`.
...
3. Queue = open issues from `.ba/plan.json`, else `gh issue list --label ba --state open --json number,body,labels`. Parse `Depends on` and `Kind`. Topological sort. Drop issues labelled `blocked` or `needs-human` (list the `needs-human` ones in the final report). Apply `max=N` if given.
...
6. Worktrees (`WT`): every issue gets its own checkout OUTSIDE the repo, so the user's IDE and index are never touched:
   ...
   Every command for that issue — writer, checkers, `gate.sh`, `prove-test.sh`, `prove-refactor.sh`, `sec-scan.sh`, git — runs with that directory as cwd. Each agent is TOLD its worktree path and must not touch any other path.
...
2. Take the first `min(PAR, |READY|)` issues. With `PAR=1` or `no-worktree`, take exactly 1. Issues of Kind `perf` and `test-split` measure time: never run them in parallel with other issues (their wave has size 1).
...
Follow `${CLAUDE_SKILL_DIR}/../implement/SKILL.md` sections 0-3 exactly (including its proof-by-KIND table), with these overrides:
...
Record `wave`, `kind` and `worktree` per issue. Checkpoint `.ba/auto.json` at EVERY step change (branch created, each writer attempt, each check result, PR opened, merged), not only at the end:
`{"issues":{"14":{"status":"in_progress|done|blocked|skipped","kind":"feature","phase":"write|verify|review|pr|merge","branch":"feat/14-x","worktree":"../repo-ba/14","wave":2,"attempt":2,"pr":21,"reason":""}}}`
...
Needs you: #17 rotate DB password, #18 Play Console data safety form
